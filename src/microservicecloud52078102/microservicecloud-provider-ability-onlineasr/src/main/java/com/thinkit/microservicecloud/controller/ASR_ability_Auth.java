...
public class ASR_ability_Auth {
    ...
    private Logger logger = LoggerFactory.getLogger(ASR_ability_Auth.class);

    @RequestMapping(value = "/provider/ability/asrauth",method = RequestMethod.POST)
    public ResponseAsrBody asr(@RequestBody RequestAsrBody info){
        ...
        synchronized (this){
            TokenJson tokenJson  = hget(info.getAuth());
            if(tokenJson==null){
    ...
    public TokenJson hget(String key){
        ...
        if(entries==null || entries.size()==0){
            ...
            tokenJson.setAuth_thread((int)entries.get("auth_thread"));
            tokenJson.setAccess_thread((int)entries.get("access_thread"));
            tokenJson.setAccess_sids((List<String>)entries.get("access_sids"));
            return  tokenJson;
        }
    ...
    public void hset(String key,TokenJson info){
        HashMap<String,Object> map = new HashMap<String,Object>();
        map.put("access_thread",info.getAccess_thread());
        map.put("access_sids",info.getAccess_sids());
        map.put("auth_thread",info.getAuth_thread());
